...
A Proxmox VM on the user's home LAN (`ssh ubuntu@192.168.1.67`, hostname `VM101`) runs the **actual live bot** — real Telegram alerts, a Flask dashboard on port 5000, Postgres in Docker. Its `~/Morpheus` git checkout is on `main`, **28 commits ahead of `origin/main`, never pushed, and unrelated to this local repo's commit history** (this repo's Modules 1-5 were rebuilt from scratch against `MockTwitterClient` without knowing the VM existed). Ported so far (all 2026-08-06): **StockTwits** collector (`src/collector/stocktwits_client.py`, `config/symbols.yaml`); the **real alerting mechanism, a periodic Telegram recap** (`src/alerts/recap_service.py` — strong signals by account/ticker every 2h, digest every 30min, daily top-10 — runs alongside `AlertService`'s cooldown-based individual alerts, not instead of it); **yfinance news collector** (`src/collector/yfinance_news_client.py`, `config/news_symbols.yaml` — same `CollectorService` pattern as StockTwits, third instance); **short-seller report collector** (`src/collector/shortseller_client.py` + `shortseller_service.py`, `config/shortsellers.yaml` — different shape, scrapes HTML directly, no `RawTweet`/NLP, sends an immediate Telegram alert outside the recap system; the VM's Playwright/Xvfb Cloudflare-bypass path was **not** ported since it's confirmed non-functional even on the VM itself — playwright is uninstalled there). Not ported: account reliability ranking (this repo's `src/ml/` Module 4 covers it more cleanly), the dashboard, the miscalibrated "Phase 2 hybrid ML" layer (see the VM's own CLAUDE.md).

A full capability-by-capability merge audit (what's on each side, what to port, in what order) was produced 2026-08-06 as a Claude Artifact — ask the user if they still have the link, or redo the comparison by reading both repos if not.
...
**StockTwits 403 diagnosed 2026-08-06 — confirmed permanent, not a code issue.** The entire `stocktwits.com` domain (not just the API) sits behind a Cloudflare managed challenge (`cf-mitigated: challenge`, "Just a moment..." page) — verified with full browser-like headers, same result. StockTwits' official developer API is closed to new signups. Same defense class that already killed the VM's Citron/Ningi short-seller scraping — not fixable client-side. `StockTwitsClient`/`config/symbols.yaml` were left in place (clean, tested code, harmless if the block ever lifts) rather than deleted.

**Replacement: `src/collector/apewisdom_client.py` + `src/collector/trending_service.py`** (ported 2026-08-06, after checking Finnhub — needs a paid-signup API key, skipped for now). ApeWisdom (`apewisdom.io`, free, no key) gives an aggregated Reddit/4chan mention-count ranking per ticker, not individual messages — structurally incompatible with the `RawTweet`/NLP pipeline, so it's a standalone `TrendingService` (same shape as `ShortSellerService`, but *does* touch the DB via `TrendingSnapshot`: immediate Telegram alert on a mention spike ≥3x vs 24h-ago, cooldown persisted in `data/trending_seen.json`). Watch for `mentions_24h_ago` arriving as JSON `null` (new tickers) — `.get(key, 0)` doesn't catch that, use `.get(key) or 0`.

**`TrendingSnapshot` (added 2026-08-06)** tracks whether a mention spike actually predicts a price move — same idea as `MarketSnapshot`/`SnapshotScheduler` (price at t=0, then 1h/4h/24h/7d, % change) but standalone since it's not tied to a `Tweet`. `TrendingService._update_pending_snapshots()` resolves the windows (reuses `WINDOWS` from `market/snapshot_scheduler.py`), `--trending-report` prints win_rate/avg_change once enough data exists. Found and fixed a real pre-existing bug while wiring this up: `StockFetcher._fetch_price` (`src/market/fetcher.py`) fell back to 1-minute intraday bars (`interval="1m"`) which yfinance frequently returns empty for ("possibly delisted" — misleading message, not an actual delisting) — switched to daily bars, verified against several real tickers. `requirements.txt` also still had `yfinance==0.2.40` pinned while `1.4.0` was the actually-installed/working version — corrected.
